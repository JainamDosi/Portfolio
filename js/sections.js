import { pacman, pellets, ghosts, TILE_SIZE } from "./maze.js";
import { openSectionModal } from "./modal.js";
...
export function checkCollisions() {
  pellets.forEach((pellet) => {
    if (!pellet.eaten) {
      ...
      if (dist < pacman.radius + TILE_SIZE / 8) {
        pellet.eaten = true;
        score += 10;
        updateScoreDisplay();
      }
  ...
  ghosts.forEach((ghost) => {
    ...
    if (dist < pacman.radius + ghost.radius - TILE_SIZE / 5) {
      openSectionModal(ghost.id, sectionContent);
    }
  });
}
